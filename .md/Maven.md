...
# 1、Maven环境搭建
## 1.1Maven下载
打开Maven官网：**https://maven.apache.org/what-is-maven.html**
点击Download-->Binary zip archive 中的apache-maven-x.x.x.x
## 1.2Maven安装
下载后的安装包直接解压。
建议：下载的编程软件放在同一个文件夹下

## 1.3Maven环境变量的配置
打开环境变量配置，在环境变量中进行不如下配置：

- M2_HOME：maven目录下的bin目录
- MAVEN_HOME：maven目录
- 在系统中的path中的配置：%MAVEN_HOME%\bin
## 1.4 Maven测试是否安装成功
管理员测试，使用**mvn -version**
## 1.5 阿里云镜像
由于Maven是外网的软件，国内与国外之间有一道防火墙，所以使用阿里云镜像
在Maven的conf目录下的settings.xml文件中找到<mirrors></mirrors>在里面添加以下代码
```xml
<mirrors>
	<mirror>
	  <id>aliyunmaven</id>
...
```
## 1.6本地仓库
在本地建立一个本地仓库，打开maven文件夹，在文件夹内建立一个maven-repo文件夹
在Maven的conf目录下的settings.xml文件中找到
```xml
  <!-- localRepository
   | The path to the local repository maven will use to store artifacts.
   |
   | Default: ${user.home}/.m2/repository
  <localRepository>/path/to/local/repo</localRepository>D:\develop\apache-maven-3.6.3\maven-repo
  <localRepository></localRepository>
  -->
  <localRepository>maven-repo路径</localRepository>
```
## 1.7IDEA 中的操作
打开IDEA，创建一个maven项目
点击 file=>settings=>
![在这里插入图片描述](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/455da6074d1f4c3da67e6e2b20f8b32c~tplv-k3u1fbpfcp-zoom-1.image)
修改箭头所指的位置，
**第一行为maven解压的位置**
**第二行为maven中conf\settings.xml文件的位置**
**第三行为1.6中maven创建的本地仓库的位置，至此以后用到的jar包就会保存在本地的maven中，如果没有，就会自动下载，然后保存在本地的maven仓库中。**
...
![image.png](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/33aed7ffda504918967cebc44e7023cc~tplv-k3u1fbpfcp-watermark.image?)

![image.png](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/446fa71575ab42309d2eb444abf99185~tplv-k3u1fbpfcp-watermark.image?)
...
| 内置属性     | ${内置属性名}              | ${version} ${basedir}      |
| Setting属性  | ${setting.属性}            | ${setting.localRepository} |
...
`mvn 指令 -P 环境定义id   | 实例：mvn install -P pro_env`
...
![image-20221104144445535](C:\Users\Yue\AppData\Roaming\Typora\typora-user-images\image-20221104144445535.png)

首先创建了**yu-common-util**模块，但是，后来删了一次，然后又重新创建yu-common-util模块，虽然依赖已经被引入，但是类中无法引入。

**造成这个的原因可能是idea忽略了maven模块**，可以尝试如下解决方法：菜单file中选择setting，搜索maven，然后选择Ignored Filess,看右边的面板中变灰的maven模块是否处于勾选状态。勾选表示忽略了这个模块的pom文件。取消勾选即可解决。

![image-20221104145300125](C:\Users\Yue\AppData\Roaming\Typora\typora-user-images\image-20221104145300125.png)